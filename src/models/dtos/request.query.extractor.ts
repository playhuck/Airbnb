...
export class RequestQueryExtractor<T> {
   ...
   * 1. string
   */
  protected getStringFromQuery(
    value: string | string[] | ParsedQs | ParsedQs[] | undefined,
    target: T
  ): string {
    if (value === undefined) {
      throw new BadRequestException(
        `Query의 ${target}은 undefiend 여서는 안됩니다.`
      );
    } else if (typeof value === "string") {
      return value;
    } else {
      throw new BadRequestException(
        `Query의 ${target} 인자의 형태가 이상합니다. ${JSON.stringify(value)}`
      );
    }
  }
  ...
  protected getStringArrayFromQuery(
    ...
    if (value === undefined) {
      throw new BadRequestException(
        `Query의 ${target}은 undefined 여서는 안됩니다.`
      );
    } else if (typeof value === "string") {
   ...
   * 1. 중복이 배제된 string[]
   */
  protected getStringSetFromQuery(
    value: string | string[] | ParsedQs | ParsedQs[] | undefined,
    target: T
  ): string[] {
    if (value === undefined)
      throw new BadRequestException(
        `Query 의 ${target} 은 undefined 이여서는 안됩니다.`
      );
    else if (typeof value === "string")
      return [...new Set(value.split(",").filter((v) => v))];
    else
      throw new BadRequestException(
        `Query 의 ${target} 인자의 형태가 이상합니다. ${JSON.stringify(value)}`
      );
  }
